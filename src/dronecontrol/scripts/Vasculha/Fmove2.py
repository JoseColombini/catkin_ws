...
def rel_Goto (x, y, z):
    global goal_pose
    x = goal_pose.pose.position.x + x
    y = goal_pose.pose.position.y + y
    z = goal_pose.pose.position.z + z
    set_position(x, y, z)
...
#ALTO INDICE DE ERRO, DRONE BALANCA PRA KRL
def rel_spd_Goto (x, y, z):
    global goal_pose
    goal_pose.pose.position.x = goal_pose.pose.position.x + x
    goal_pose.pose.position.y = y + goal_pose.pose.position.y
    goal_pose.pose.position.z = z + goal_pose.pose.position.z
    ...
    while not secure_accuracy(Drone_pose, goal_pose):
        print ("passo 1")
        if (abs(goal_pose.pose.position.x - Drone_pose.pose.position.x) > accuracy):
            print("aqui era para entrar")
            vx =  math.copysign(spd, goal_pose.pose.position.x - Drone_pose.pose.position.x)
        if (abs(goal_pose.pose.position.y - Drone_pose.pose.position.y) > accuracy):
            vy =  math.copysign(spd, goal_pose.pose.position.y - Drone_pose.pose.position.y)
        if (abs(goal_pose.pose.position.z - Drone_pose.pose.position.z) > accuracy):
            vz =  math.copysign(spd, goal_pose.pose.position.z - Drone_pose.pose.position.z)
        set_speed(vx, vy, vz)
        rate.sleep()
...
def Xrel_spd_Goto (x):
    ...
    while not secure_accuracy(Drone_pose, goal_pose):
        print ("passo 1")
        if (abs(goal_pose.pose.position.x - Drone_pose.pose.position.x) > 0.5):
            abs_Goto(goal_pose.pose.position.x, goal_pose.pose.position.y, goal_pose.pose.position.z)
        else:
            if (abs(goal_pose.pose.position.x - Drone_pose.pose.position.x) > accuracy):
                print("aqui era para entrar")
                vx =  math.copysign(vx, goal_pose.pose.position.x - Drone_pose.pose.position.x)
            if (abs(goal_pose.pose.position.y - Drone_pose.pose.position.y) > accuracy):
                vy =  math.copysign(vy, goal_pose.pose.position.y - Drone_pose.pose.position.y)
            if (abs(goal_pose.pose.position.z - Drone_pose.pose.position.z) > accuracy):
                vz =  math.copysign(vz, goal_pose.pose.position.z - Drone_pose.pose.position.z)
            set_speed(vx, vy, vz)
        rate.sleep()
