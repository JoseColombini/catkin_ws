...
// TEM QUE FAZER ISSO DIREITO PORRA
...
void verification(){
  ...
  ros::NodeHandle nh;
	...
	while(node_message == 1){
      ...
      /** get gps location and save in file**/
			return_to_home();
		}
	}
}
...
{
	double search_height = 4.0;
  double r = search_height*sin(51* 	3.14159265358979323846/180);
  double step = 1.5*r;
  ...
  if(File.is_open()) {
      File >> SEARCH_POSITION.latitude;
      File >> SEARCH_POSITION.longitude;
  }
  ...
  pose.pose.position.z = search_height;
  ...
  HOME_POSITION = gps_pose;
	...
	if (fabs(position_now[2] - search_height) < ERROR){
    //while ()
      ...
      set_home.call(NEW_HOME);
        /****************************************/
    double width = 2;
    double total_width = 10; //total width of search rectangle
    double total_height = 15;
    //set_position(i*width, (j+1)*total_height, search_height);5; //total height of search rectangle
    // for(double s = 0; (s <= 10); s += step)
    //     set_position(s, 0.0, search_height);
    double j = 1.0;
    for (int i = 0; i < total_width/width; i++) {
      set_position(i*width, j*total_height, search_height);
      j=1.0;
      set_position(i*width, j*total_height, search_height);
      i++;
      set_position(i*width, j*total_height, search_height);
